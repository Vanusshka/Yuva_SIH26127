...
function ApiStatus() {
  const [status, setStatus] = useState<'checking'|'waking'|'connected'|'disconnected'>('checking')
  ...
  useEffect(() => {
    ...
    const check = () => {
      fetchHealth()
        ...
        .catch(() => {
          failCount++
          // Show 'waking' after 1st failure (Render cold start takes 30-50s)
          // Only show 'disconnected' after 3 consecutive failures (~3 min)
